...
class TestPublisherHandler:
    ...
    def publisher_service(self, ):
        with patch('app.services.publisher_service.setup_logger') as mock_setup_logger:
            mock_setup_logger.return_value = MagicMock()
            return PublisherService()

    @pytest.mark.asyncio
    async def test_safe_publish_success(self, publisher_service):
        queue_name = 'test_queue'
        message = {'key': 'value'}

        publisher_service.rabbitmq_util.publish_message = AsyncMock()
        await publisher_service.safe_publish(queue_name, message)

        publisher_service.rabbitmq_util.publish_message.assert_awaited_once_with(queue_name, message)

    @pytest.mark.asyncio
    async def test_safe_publish_channel_closed(self, publisher_service):
        queue_name = 'test_queue'
        message = {'key': 'value'}

        publisher_service.rabbitmq_util.publish_message = AsyncMock(side_effect=[
            ChannelClosed(reply_code=200, reply_text="Channel closed"),
            None
        ])
        publisher_service.rabbitmq_util.reopen_channel = AsyncMock()

        await publisher_service.safe_publish(queue_name, message)

        publisher_service.rabbitmq_util.reopen_channel.assert_awaited_once()
        assert publisher_service.rabbitmq_util.publish_message.await_count == 2

    @pytest.mark.asyncio
    async def test_safe_publish_connection_closed(self, publisher_service):
        queue_name = 'test_queue'
        message = {'key': 'value'}
    ...
    @pytest.mark.asyncio
    async def test_safe_publish_general_exception(self, publisher_service):
        queue_name = 'test_queue'
        message = {'key': 'value'}

        publisher_service.rabbitmq_util.publish_message = AsyncMock(side_effect=Exception('Test exception'))
